...
test.describe('menu', () => {
  ...
  test('active menu item should be highlighted', async ({ page }) => {
    ...
    await expect(shared.file).toHaveClass(/bg-surface/);
    await expect(page.getByRole('heading', { name: /my files/i })).toBeVisible();

    await shared.archive.click();
    await expect(shared.file).not.toHaveClass(/bg-surface/);
    await expect(shared.archive).toHaveClass(/bg-surface/);
    await expect(page.getByRole('heading', { name: /archived files/i })).toBeVisible();

    await shared.trash.click();
    await expect(shared.archive).not.toHaveClass(/bg-surface/);
    await expect(shared.trash).toHaveClass(/bg-surface/);
